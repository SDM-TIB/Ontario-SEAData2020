...
def get_options(argv):
    try:
        opts, args = getopt.getopt(argv, "h:c:q:t:s:r:j:p:a:b:")
    ...
    return (configfile, queryfile, tempType, isEndpoint, plan,
            adaptive, withoutCounts, printResults, result_folder, joinlocally, alpha, beta)
...
def main(argv):
    ...
    (configfile, queryfile, buffersize, isEndpoint, plan, adaptive, withoutCounts, printResults, result_folder,
     joinlocally, alpha, beta) = get_options(argv[1:])
    try:
        cfg.alpha = alpha
        cfg.beta = beta
